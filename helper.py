...
def medal_tally(df):
    ...
    medal_tally['Total'] = medal_tally['Silver'] + medal_tally['Gold'] + medal_tally['Bronze']
...
def data_over_time(df,col):
    nations_over_time = df.drop_duplicates(['Year', col])['Year'].value_counts().reset_index().sort_values(
        by='index')
    nations_over_time.rename(columns={'Year': col, "index": "Year"}, inplace=True)
...
def most_successfull_countrywise_athletes(df, country):
    temp_df = df.dropna(subset=['Medal'])
